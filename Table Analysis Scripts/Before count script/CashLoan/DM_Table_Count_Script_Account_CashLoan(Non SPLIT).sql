...
/*************************************************************************
	...
	Script Name  : DM_Table_Count_Script_Account_CashLoan(Non SPLIT).sql
	Country		 : Dominica
        ...
        BEGIN
            SET @SQL += 
                ...
                ' WHERE acctno IS NOT NULL AND acctno NOT IN (' + CHAR(39) + CHAR(39) + ') AND LEN(acctno) > 11 
					AND acctno in (SELECT acctno FROM dbo.acct WHERE acctno IN (SELECT acctno FROM dbo.cashloan WITH (NOLOCK)))
					UNION ALL ';
        END
        ELSE
        BEGIN
