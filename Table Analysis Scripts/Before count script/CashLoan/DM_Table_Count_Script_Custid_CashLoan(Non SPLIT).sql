...
/*************************************************************************
	...
	Script Name  : DM_Table_Count_Script_Custid_CashLoan_(Non SPLIT).sql
	Country		 : Dominica
	Created By   : Yash Sagar Mehta             
	Description  : This script take count of cashloan entries from tables (for non splitted DB)         
	Date         : 15-03-2025           
    ...
    BEGIN
        ...
        BEGIN
            SET @SQL += 
                ...
                ' WHERE custid in (SELECT custid FROM dbo.customer WHERE custid IN 
				(SELECT custid FROM dbo.cashloan WITH (NOLOCK)))
					UNION ALL ';
        END
        ELSE
        BEGIN
