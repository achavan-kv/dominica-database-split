...
/*************************************************************************
	...
	Script Name  : DM_Table_Count_Script_Account_Retail.sql
	Country		 : Dominica
	Created By   : Yash Sagar Mehta             
	Description  : This script take count of retail entries from tables (Non SPLIT)        
	Date         : 15-03-2025           
        ...
        BEGIN
            SET @SQL += 
                ...
                ' WHERE acctno IS NOT NULL AND acctno NOT IN (' + CHAR(39) + CHAR(39) + ') AND LEN(acctno) > 11 
					AND acctno in (SELECT acctno FROM dbo.acct WHERE acctno NOT IN (SELECT acctno FROM dbo.cashloan WITH (NOLOCK)))
					UNION ALL ';
        END
        ELSE
        BEGIN
